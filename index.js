...
const BOT_LAUNCH_INTERVAL = { min: 10000, max: 15000 };
...
handleMessage(msg, bot, username) {
    ...
    if (
        msg.includes('You are already connected to this server!') ||
        msg.trim() === ''
    ) {
        return;
    }
    ...
    if (
        !botData.loggedIn &&
        (
            msg.includes('/login <пароль>') ||
            msg.includes('Пожалуйста, войдите') ||
            msg.includes('войдите с помощью')
        )
    ) {
        botData.loggedIn = true;

        setTimeout(() => {
            if (botData.isActive) {
                bot.chat('/login lalka');
                console.log(`${username}: отправил /login`);
            }
        }, getRandomInt(1000, 3000));
    ...
    // Успешный вход
    if (
        msg.includes('Вы успешно вошли') ||
        msg.includes('Вход выполнен успешно')
    ) {
        setTimeout(() => {
            if (botData.isActive) {
                bot.chat('/aboba888d');
